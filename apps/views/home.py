...
from apps.models import Donor, Blood, PSL, Type_PSL, Order, Users, Analysis
...
def home(request):
    ...
    if is_donor and not ingroups:
        donor_info_display = True
    else:
        donor_info_display = False
    ...
    elif donor_info_display:
        connected_donor = request.user.donor
        connected_donor_id = connected_donor.id
        donor_infos = Donor.objects.get(id=connected_donor_id)
        donor_bloods = Blood.objects.filter(donor__id=connected_donor_id)
        donor_analysis = Analysis.objects.filter(blood__donor__id=connected_donor_id)

        context = {"ingroups": ingroups, "donor_infos": donor_infos, "donor_bloods": donor_bloods, "donor_analysis": donor_analysis, "donor_info_display": donor_info_display}
    else:
        context = {"ingroups": ingroups}

    print(f"DDDDDDDD {donor_info_display}\n\n {ingroups}")
